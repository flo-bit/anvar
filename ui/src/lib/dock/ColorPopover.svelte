<script lang="ts">
	...
	// pointer is the source of truth while dragging (see Slider.svelte)
	let dragHue = $state<number | null>(null);
	const hue = $derived(dragHue ?? look.hue);
	const rad = $derived((hue * Math.PI) / 180);
	const handleX = $derived(50 + 44 * Math.sin(rad));
	const handleY = $derived(50 - 44 * Math.cos(rad));

	const send = rateLimit((h: number) => look.setHueSat(h, look.sat), 80);
	...
	function ringDown(e: PointerEvent) {
		...
		trackDrag(
			...
			(ev) => {
				...
				dragHue = Math.round((ang + 360) % 360);
				send(dragHue);
			},
			() => {
				if (dragHue !== null) send(dragHue);
				dragHue = null;
			}
		);
	}
